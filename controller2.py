...
with open('c2_role.csv', 'w') as file:
    file.write(str(['SLAVE', 'MASTER']))
    file.close()
...
def c2_counter():
        extracted_data = None
        with open('packet_in_counter_2.csv', 'r') as csvfile:
                ...
                # Check the type of the data
                        data_type = type(extracted_data)
                        #print("Extracted Data from c2:", extracted_data)
                        #print("Data Type:", data_type)
                        break  # Exit loop after reading the first row
...
class SimpleSwitch13_c2(app_manager.RyuApp):
    OFP_VERSIONS = [ofproto_v1_3.OFP_VERSION]
    def __init__(self, *args, **kwargs):
        super(SimpleSwitch13_c2, self).__init__(*args, **kwargs)
        self.mac_to_port = {}
        self.gen_id = 0
        self.role_string_list = ['nochange', 'equal', 'master', 'slave', 'unknown']
        self.datapath = []
        #   Initial roles of C2 controller
        self.role = ['SLAVE', 'MASTER']
    ...
    def open_server(self, event):
        ...
        try:
            server.bind((TCP_IP, TCP_PORT2))
        except:
            print("Binding failed")
        print("Controller 2 TCP opened succesfully")
        server.listen(1)
        conn, addr = server.accept()


        while True:
            #   Receiving data from controller C1
            data = conn.recv(1024)
            if data:
                ...
                for i in data_splitted:
                        data_list_splitted.append(int(i))
                if len(data_list_splitted)>5:
                        c1_data = data_list_splitted[-1]
                        with open('packet_in_counter_1.csv', 'w') as csv_file:
                                csv_file.write(str(c1_data))
                                csv_file.close()
            ...
            bw_list = self.bandwidth_list
            if len(bw_list)>5:
                bw_list.pop()
            if c2_counter():
                c2_data = c2_counter()
                c2_data = int(c2_data[0])
                bw_list.append(c2_data)
            bandwidth_string = ','.join(str(e) for e in bw_list)
            #print('This data will be sent to controller 1: ',bandwidth_string)
            b = bytearray()
    ...
    def bw_limit_exceeded(self, index_list, src, dst, datapath):
        ...
        #########################################################################################################
        if len(bw_list_intermediate_int)>5:
                bw_list_intermediate_int.pop()
        #########################################################################################################
        #print(' bw_list_intermediate_int RECEIVED:  ', bw_list_intermediate_int)


        if len(self.bandwidth_list)>5:
                self.bandwidth_list.pop()
    ...
    def _monitor(self):
        ...
        thread1 = Thread(target=self.open_server, args=(event,))
        thread1.start()
        ...
        while True:
            ...
            #   Load balancing control plane
                ...
                if os.path.getsize('packet_in_counter_1.csv') > 0 and os.path.getsize('packet_in_counter_2.csv') > 0:
                    ...
                    print("packet_in_counter_c1_curr, packet_in_counter_c2_curr", packet_in_counter_c1_curr,
                          packet_in_counter_c2_curr)
                    ...
                    #   Checking if there is an overloaded controller and switching controller roles
    ...
    def _packet_in_handler(self, ev):
        ...
        packet_in_counter_c2 = packet_in_counter_c2 + 1
        #   Updating packet_in counter files
        f = open("packet_in_counter_2.csv", "r+b")
        f.seek(0)
        f.write(str(packet_in_counter_c2))
        f.close()
        ...
        dpid = datapath.id
        self.logger.info("packet in switch %s %s %s %s count %d", dpid, src, dst, in_port, packet_in_counter_c2)
        ...
        if (out_port != ofproto.OFPP_FLOOD):
            ...
            if self.role == ['SLAVE', 'MASTER']:
                ...
                elif (src == '00:00:00:00:00:04' and dst == '00:00:00:00:00:01'):
                    print ("h4-h1 !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
                    index_list = [0, 3, 4]
                    self.bw_limit_exceeded(index_list, src, dst, datapath)
                elif (src == '00:00:00:00:00:03' and dst == '00:00:00:00:00:02'):
                    print ("h3-h2 !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
                    index_list = [1, 2, 4]
                    self.bw_limit_exceeded(index_list, src, dst, datapath)
                elif (src == '00:00:00:00:00:04' and dst == '00:00:00:00:00:02'):
                    print ("h4-h2 !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
                    index_list = [1, 3, 4]
                    self.bw_limit_exceeded(index_list, src, dst, datapath)
                elif ((src == '00:00:00:00:00:03' and dst == '00:00:00:00:00:04') or (
                        src == '00:00:00:00:00:04' and dst == '00:00:00:00:00:03')):
                    print ("h3-h4/h4-h3 !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
                    index_list = [2, 3]
                    self.bw_limit_exceeded(index_list, src, dst, datapath)
