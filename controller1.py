...
def c1_counter():
        extracted_data = None
        with open('packet_in_counter_1.csv', 'r') as csvfile:
...
class SimpleSwitch13_c1(app_manager.RyuApp):
    ...
    def open_client(self, event):
        ...
        while True:
            ...
            if c1_counter():
                c1_data = c1_counter()
                c1_data = int(c1_data[0])
                bw_list.append(c1_data)
            ...
            if data:
                ...
                if len(data_list_splitted)>5:
                        c2_data = data_list_splitted[-1]
                        with open('packet_in_counter_2.csv', 'w') as csv_file:
                                csv_file.write(str(c2_data))
                                csv_file.close()
    ...
    def bw_limit_exceeded(self, index_list, src, dst, datapath):
        ...
        #   If bandwidth is not enough drop the flow and remove the flow rules from the flow table
            ...
            print("BW is not enough >>>>>>>>>> Drop the flow and Remove")
